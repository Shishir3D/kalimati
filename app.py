...
def index():
    try:
        ...
        fig_avg.update_layout(
            title={
                'text': "Average Prices Over Years (Top 10 Commodities)",
                'x': 0.5,
                'xanchor': 'center',
        ...
        fig_avg.update_xaxes(showgrid=True, gridwidth=1, gridcolor='rgba(128,128,128,0.2)')
        fig_avg.update_yaxes(showgrid=True, gridwidth=1, gridcolor='rgba(128,128,128,0.2)')
        ...
        fig_scatter.update_layout(
            title={
                'text': "Min vs Max Prices (Top 10 Commodities)",
                'x': 0.5,
                'xanchor': 'center',
                'font': {'size': 24, 'color': '#2c3e50'}
            },
        ...
        scatter_chart = pio.to_html(fig_scatter, full_html=False, include_plotlyjs='cdn')
        ...
        if request.method == "POST":
            ...
            if budget:
                recipe_recommendation = get_recipe_recommendation(budget, top_commodities[:5])
            else:
